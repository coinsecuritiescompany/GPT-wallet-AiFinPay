...
- Casper (the 13th network) reads native CSPR via the non-EVM `query_balance` JSON-RPC method, using `main_purse_under_public_key` and 9 decimals. The default `https://node.mainnet.casper.network/rpc` endpoint is keyless. The faster `https://node.cspr.cloud/rpc` option requires a server-side `CASPER_RPC_AUTH` value. An unfunded Casper account (no main purse yet) resolves to a zero balance rather than an outage.
