...
type Step = "welcome" | "phrase" | "verify" | "password" | "restore" | "unlock" | "ready";
...
export function VaultApp() {
  ...
  return <main className="vault-shell">
    ...
    {step === "unlock" && <section className="vault-panel"><span className="vault-kicker">WELCOME BACK</span><h2>Unlock your wallet</h2><p>This device already holds your encrypted AiFinPay vault. Enter your password to continue{oauthRequest || pairToken ? " and reconnect to ChatGPT" : ""}. Your recovery phrase never leaves this device.</p><label className="vault-field">Password<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void unlock(); }} autoComplete="current-password" autoFocus /></label>{error && <p className="vault-error">{error}</p>}<button className="primary" disabled={busy} onClick={() => void unlock()}>{busy ? "Unlocking…" : "Unlock"}</button><button className="text-button" onClick={() => { setError(""); setPassword(""); setStep("restore"); }}>Forgot password? Restore from recovery phrase</button><button className="danger-link" onClick={remove}>Remove vault from this device</button></section>}
