...
export function VaultApp() {
  ...
  const save = async (source = mnemonic) => {
    ...
    if (normalizeVaultPassword(submittedPassword) !== normalizeVaultPassword(submittedConfirmation)) { setError("Passwords do not match."); return; }
    setBusy(true); setError("");
    const previous = localStorage.getItem(STORAGE_KEY);
    let savedAndVerified = false;
    try {
      const next = await encryptVault(source, submittedPassword);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(next));
      const persisted = parseEncryptedVault(JSON.parse(localStorage.getItem(STORAGE_KEY) ?? "null"));
      if (!persisted) throw new Error("The saved vault could not be read back.");
      ...
      const signed = transaction.kind === "SOLANA"
        ...
        : transaction.kind === "NEAR"
          ...
          : transaction.kind === "APTOS"
            ...
            : transaction.kind === "CASPER"
              ? signCasperTransaction(mnemonicRef.current, transaction)
  ...
  const remove = () => {
    if (!removeArmed) {
      setRemoveArmed(true);
      window.setTimeout(() => setRemoveArmed(false), 5_000);
      return;
    }
    mnemonicRef.current = "";
  ...
  return <main className="vault-shell">
    ...
    {step === "welcome" && <section className="vault-panel"><span className="vault-kicker">NON-CUSTODIAL · 13 MAINNETS</span><h1>Your wallet for ChatGPT</h1><p>Create one recovery phrase for EVM, Solana, NEAR, Aptos and Casper addresses. Keys are generated and encrypted on this device only.</p><div className="recovery-note"><span aria-hidden="true">✦</span><div><strong>Your recovery phrase will be unique</strong><p>After you tap Create wallet, this device generates 12 or 15 random security words. Every new wallet receives a different phrase.</p></div></div><div className="choice"><button className={wordCount === 12 ? "selected" : ""} onClick={() => setWordCount(12)}>12 words</button><button className={wordCount === 15 ? "selected" : ""} onClick={() => setWordCount(15)}>15 words</button></div><button className="primary" onClick={create}>Create wallet</button><button className="text-button" onClick={() => { setError(""); setStep("restore"); }}>Restore existing wallet</button></section>}
