...
# before compiling so Android receives the current bridge instead of wallet-v25.
RUN sed -i 's|ui://aifinpay/wallet-v23.html|ui://aifinpay/wallet-v26.html|g; s|{ length: 22 }|{ length: 25 }|g' apps/mcp-server/src/tools/register-tools.ts
RUN npm run build
